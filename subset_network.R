...
append_style <- function(gpkg_path, qml_dir = system.file("qml", package = "hydrofabric"), layer_names) {
    ...
    sf::st_write(style_rows, gpkg_path,
        layer = "layer_styles", append = FALSE,
        quiet = TRUE
    )
    return(gpkg_path)
}
...
get_fabric <- function(VPU,
    ...
    xx <- aws.s3::get_bucket_df(bucket = dirname(base_s3), prefix = basename(base_s3), region = "us-west-2") |>
...
subset_network <- function(id = NULL,
    ...
    net <- glue::glue(base_s3, "conus_net.parquet") |>
        arrow::open_dataset() |>
    ...
    if (!is.null(loc)) {
        comid <- nhdplusTools::discover_nhdplus_id(
            point = sf::st_point(c(loc[1], loc[2])) |>
                sf::st_sfc(crs = 4326)
        )
    }
    ...
    if (is.null(net)) {
        ...
        index <- sf::st_transform(v, sf::st_crs(xx)) |>
            sf::st_intersects(xx) |>
            lengths() |>
            which()

        vpuid <- v$VPUID[index > 0]
    } else {
        vpuid <- dplyr::filter(net, id == origin | toid == origin) |>
            dplyr::pull(vpu) |>
            unique()
    }
    ...
    if (!is.null(net)) {
        sub_net <- dplyr::filter(net, vpu == vpuid) |>
        ...
        sub_net <- dplyr::tbl(db, lyrs[grepl("flowline|flowpath", lyrs)]) |>
            dplyr::select(
                dplyr::any_of(
                    c("id", "toid", "COMID", "toCOMID", "ID", "toID", "member_COMID")
                )
            ) |>
            dplyr::collect() |>
